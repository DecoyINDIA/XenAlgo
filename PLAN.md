...
**Autonomous NSE swing-trading system on Fyers. Single user. Real capital. Zero human intervention in the loop — maximum human control over the loop.**
...
**Objective:** A fully autonomous swing-trading bot that runs the three validated alphas (`std30`, `alpha_027`, `alpha_062`) on the NIFTY-500 universe via Fyers, with institutional-grade guardrails, a secure real-time web console, and complete auditability. Phase 2 adds a memory/learning layer.
...
| Fyers API rate limits | 10/s · 200/min · 100,000/day | Client-side order governor hard-caps at 2/s, far below broker and SEBI limits. |
...
```
  ...
  Fyers REST ◄──────────┤  BrokerGateway (SDK/REST, token-bucket,     │
  Fyers Order WS ◄──────┤    correlationId/tag idempotency, retries)  │
  REST orderbook poll ──►  FillListener (WS + poll, dual channel)      │
                        │                                             │
...
- **Fills are facts, not assumptions:** positions update only from confirmed fill events (`filledQty`, average traded price) via the Fyers Order WebSocket, with REST orderbook polling as the redundant channel. `PENDING ≠ filled` — the current code's core flaw, eliminated.
...
- **SDK posture:** pin exact `fyers-apiv3` version; keep SDK clients behind injectable wrappers and supervise WebSocket reconnects.
...
**Layer 3 — Operational:**
- **Kill switch paths:** dashboard button, Telegram `/kill` command, session-token revocation, and account-level capital cap. Fyers has no broker-side kill-switch API equivalent; this Layer-4 gap is accepted with these compensating controls.
...
**Layer 4 — Blast radius:** bot runs in a **dedicated Fyers account** funded only with allocated trading capital. Worst case is bounded by construction.
...
**Security:** **Tailscale-only. Zero public ports.** The dashboard binds to loopback or the tailnet interface; access from phone/laptop via Tailscale. No auth surface is exposed to the internet. Fyers fill delivery uses Order WebSocket plus REST orderbook polling, so there is no public postback endpoint.
...
SQLite journal + order state machine · Fyers BrokerGateway (SDK/REST boundary, rate-buckets, correlationId/tag idempotency) · dual-channel FillListener · RiskEngine with full Layer 0–2 checks · Reconciler (holdings+positions) · sleeve-based StrategyEngine · Scheduler + calendar · token manager · Telegram/Pushover alerter. **Paper mode runs this identical pipeline** (PaperBroker upgraded to consume live LTPs) — paper and live differ only at the gateway boundary.
