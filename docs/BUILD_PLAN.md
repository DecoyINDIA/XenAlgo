...
| 1.5 | **BrokerGateway (Fyers)**: place/modify/cancel + funds/holdings/positions; deterministic tag idempotency; marketable-limit; error mapping; no blind POST retry. Behind `OrderGateway`. | L | idempotent lookup; injected-client contract |
...
| 1.7 | **FillListener**: Fyers Order WebSocket + REST orderbook fallback, cumulative-fill normalization, dedup, idempotent journal application. | M | duplicate fill no-op; WS drop → REST recovers |
...
| 2.5 | Tailscale-only deployment; loopback/tailnet bind guard; no public inbound broker endpoint. | M |
...
the initial 10% live-capital stage, but does not call Fyers, mutate config, fund the
